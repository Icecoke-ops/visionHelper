...
- `scripts/`：纯命令行/编程接口的核心工具包。
  - 顶层模块（如 `scripts.auto_annotate`）只承担 **CLI 门面** 职责
    （argparse + 参数校验 + 退出码），并 re-export 公开 API。
  - 真正的业务实现统一收纳于 `scripts/core/` 子包，由门面与 `scripts.api`
    懒加载，保证 `import scripts` 自身**零副作用、零重依赖**。
- `gui/`：基于 PyQt5 的桌面图形界面，通过 **子进程** 调用
  `python -m scripts.xxx` 执行后台任务，从而把 `torch` / `ultralytics`
  等重型依赖完全隔离在用户自选的 Python 解释器中。
...
- **CLI 门面 / 业务实现分层**：所有耗时业务实现迁移至
  `scripts/core/<feature>.py`，`scripts/<feature>.py` 仅保留参数解析与
  入口逻辑；上层通过 `scripts.api` 调用更稳定的 API。
- **`scripts/__init__.py` 与 `scripts/core/__init__.py` 零副作用**：使用
  `__getattr__` 懒加载子模块，避免 GUI 打包态下只触碰
  `scripts._common` 也被牵连导入 `torch` / `cv2`，造成闪退。
...
`scripts/core/annotation_type.py` 根据 JSON 中是否包含 `auto_annotated_time`
...
> 表格中"实现位置"列指向真正的业务代码；外层文件仅为 CLI 门面。

| 工具 | CLI 门面 | 实现位置 |
| --- | --- | --- |
| 视频抽帧 | `scripts/extract_video_frames.py` | `scripts/core/extract_video_frames.py` |
| 图片去重 | `scripts/deduplicate_images.py` | `scripts/core/deduplicate_images.py` |
| 标注统计 | `scripts/annotation_stats.py` | `scripts/core/annotation_stats.py` |
| 自动标注 | `scripts/auto_annotate.py` | `scripts/core/auto_annotate.py` |
| 标注清除 | `scripts/clear_annotations.py` | `scripts/core/clear_annotations.py` |
| YOLO 数据集导出 | `scripts/export_yolo_dataset.py` | `scripts/core/export_yolo_dataset.py` |
| 模型训练 | `scripts/train_model.py` | `scripts/core/train_model.py` |
| 标注类型判定 | — | `scripts/core/annotation_type.py` |

各工具的能力概述：

- **视频抽帧 (`scripts.extract_video_frames`)**
  - 按指定帧间隔从视频中抽取画面并保存为图片。
  - 支持 JPEG、PNG、WebP、BMP、TIFF 等格式。
  - 支持指定起止时间、图片质量、文件名前缀、`--seek-mode {decode_all,seek}`、`--overwrite` 等参数。

- **图片去重 (`scripts.deduplicate_images`)**
  - 提供两种特征后端：
...
- **标注清除 (`scripts.clear_annotations`)**
...
- **YOLO 数据集导出 (`scripts.export_yolo_dataset`)**
...
- **`scripts/_common.py`**：内部公共工具（图像/标注扩展名常量、`is_image_file`、`load_annotation`、
  `resolve_image_path`、`discover_trained_models` 等），避免在各工具间重复实现。
- **`scripts/config.py`**：后端公共常量（任务类型、扩展名、CLI 输出协议标记、进度日志参数等），
  零副作用、零重依赖。
- **`scripts/logging_utils.py`**：统一 `log()` 与 `ProgressLogger`（替代 tqdm，适配 GUI 日志面板）。
...
- **主程序 (`gui/app.py`)**
  ...
  - 通过子进程调用 `python -m scripts.xxx` 执行任务，并弹出日志窗口实时展示输出。
...
- **子进程参数构造 (`gui/_proc.py`)**
  - `build_script_argv()` 把 Python 风格 kwargs 转换为 `python -m scripts.xxx --kebab-case` 形式
    的 argv，自动跳过 `None` / 空串、处理 `bool` / `list`。
...
推荐使用以下 Python 环境运行 `scripts/`（深度学习相关功能依赖较重）：
...
```bash
/home/zh/.anaconda3/envs/vision/bin/python -m scripts.annotation_stats \
...
```bash
# 仅清除"自动标注"产生的 JSON
/home/zh/.anaconda3/envs/vision/bin/python -m scripts.clear_annotations \
    /path/to/images \
    --include-auto
```
...
payload = AnnotationAPI.annotation_stats_cli(
    ...
    python_executable="/home/zh/.anaconda3/envs/vision/bin/python",
)
...
# 或者
/home/zh/.anaconda3/envs/vision/bin/python gui_main.py
```
...
│   ├── __init__.py                    # 零副作用，按需懒加载
│   ├── _common.py                     # 内部公共工具
│   ├── config.py                      # 后端公共常量
│   ├── logging_utils.py               # 统一日志 / 进度工具
│   ├── api.py                         # 对外统一 API 接口
│   ├── annotation_stats.py            # 标注统计 CLI 门面
│   ├── auto_annotate.py               # 自动标注 CLI 门面
│   ├── clear_annotations.py           # 标注清除 CLI 门面
│   ├── deduplicate_images.py          # 图片去重 CLI 门面
│   ├── export_yolo_dataset.py         # YOLO 数据集导出 CLI 门面
│   ├── extract_video_frames.py        # 视频抽帧 CLI 门面
│   ├── train_model.py                 # 模型训练 CLI 门面
│   └── core/                          # 业务实现层（被 CLI 门面 / api 调用）
│       ├── __init__.py                    # 零副作用
│       ├── annotation_type.py             # 标注类型判定
│       ├── annotation_stats.py            # 标注统计实现
│       ├── auto_annotate.py               # 自动标注实现
│       ├── clear_annotations.py           # 标注清除实现
│       ├── deduplicate_images.py          # 图片去重实现（vit / phash）
│       ├── export_yolo_dataset.py         # YOLO 数据集导出实现
│       ├── extract_video_frames.py        # 视频抽帧实现
│       └── train_model.py                 # 模型训练实现
├── gui/                           # 图形界面包（PyQt5）
│   ├── __init__.py
...
- 关键函数包含参数校验和异常处理。
- 业务实现统一放在 `scripts/core/<feature>.py`，`scripts/<feature>.py`
  保持轻量门面；新增工具时请遵循同一分层。
...
- `scripts/` 与 `gui/` 互不依赖：
  ...
  - `gui` 不应 `import scripts.api` 或核心 `scripts.core.*` 模块，
    所有耗时操作都通过 `python -m scripts.xxx` 子进程触发。
- `scripts/__init__.py` 与 `scripts/core/__init__.py` 必须保持零副作用，
