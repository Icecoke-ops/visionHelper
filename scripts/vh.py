...
if __package__ in {None, ""}:
    # 兼容 ``python scripts/vh.py`` 直接脚本启动模式：此时 sys.path[0]
    # 是 scripts/ 目录本身，需把项目根目录加入 sys.path 才能 import scripts.*。
...
ROUTES: dict[tuple[str, str], str] = {
    ("images", "import"):  "scripts.images.import_",
    ("images", "dedup"):   "scripts.images.dedup",
    ("images", "augment"): "scripts.images.augment",
    ("datasets", "stats"): "scripts.datasets.stats",
    ("datasets", "auto"):  "scripts.datasets.auto",
    ("datasets", "clear"): "scripts.datasets.clear",
    ("datasets", "export"):"scripts.datasets.export",
    ("train", "run"):      "scripts.train.train",
    ("predict", "run"):    "scripts.predict.predict",
    ("deploy", "export"):  "scripts.deploy.deploy",
}
...
def _get_main(subcommand: str, action: str) -> Callable[[List[str]], int]:
    ...
    if key not in _MAIN_CACHE:
        module_path = ROUTES.get((subcommand, action))
        if module_path is None:
            raise ValueError(f"未知路由: {subcommand} {action}")
...
def _apply_global_options(args: argparse.Namespace) -> None:
    ...
    if args.quiet:
        os.environ["VH_QUIET"] = "1"
    if args.log_file:
        os.environ["VH_LOG_FILE"] = args.log_file
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if remaining and not args.subcommand:
        parser.error(f"unexpected arguments: {' '.join(remaining)}")
    ...
    except (ValueError, ModuleNotFoundError, ImportError, AttributeError) as exc:
